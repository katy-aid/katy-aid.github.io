...
for (let i = 0; i < addiction.length; i++) {
    ...
    document.querySelector(`section.medical`).innerHTML += `
        <div class="resource">
            <a href="${r.url}" target="_blank">
                <div class="card">
                    <div id="text${i + diseases.length}" class="text">
                        <h3>${r.name}</h3>
                        <p>${r.description}</p>
                    </div>
                    <div id="site-img${i + diseases.length}" class="site-img"></div>
                </div>
            </a>
        </div>
    `;
    ...
    // Add image of resource site below its name and description
    document.querySelector(`#site-img${i + diseases.length}`).style.setProperty('background', `url("assets/img/sites/medical/${i + diseases.length}.png") no-repeat top center / cover`, 'important'); // background: imageURL repeat verticalPosition horizontalPosition / size

    // Access div containing current name and description
    let text = document.querySelector(`#text${i + diseases.length}`);

    // Adjust height of resource site image based on overall size of name and description
    document.querySelector(`#site-img${i + diseases.length}`).style.setProperty('height', `calc(100% - 20px - ${window.getComputedStyle(text).height})`);
}